    ...
    {if $item.id != "" or $type == 'new'}
        <div class="modal-body">
            ...
            <div class="form-group row">
                <div class="col-lg-12">
                    <label>{#fieldPadre#} <span class="text-danger bold">*</span> : </label>
                    <div class="input-group">
                        <select class="form-control m-select2 select2_general" name="item[oficina_id]" id="oficina_id"
                                data-placeholder="{#fieldPadreHolder#}" {$privFace.input}
                                required
                                data-fv-not-empty___message="{#glFieldRequired#}"
                        >
                            <option></option>
                            {html_options options=$cataobj.nombre selected=$item.oficina_id}
                        </select>
                    </div>
                    <span class="form-text text-black-50">{#fieldPadreMsg#}</span>
                </div>
                <div class="col-lg-12">
